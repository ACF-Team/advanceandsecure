...
if SERVER then
	...
	local function BuildDupeList()
		...
		MsgN("[AAS Dupes] + Rebuilding dupe list...")
		...
		for _, v in pairs(Dupes) do
			...
			local n = string.StripExtension(v)

			DupeList[n] = {txt = v, size = FileSize, strsize = math.Round(FileSize / 1024, 2) .. "kB"}

			MsgN("[AAS Dupes] - Added " .. n)
			count = count + 1
		end

		MsgN("[AAS Dupes] Total dupes: " .. count)
	end
	BuildDupeList()
	...
	local function DupeMenu()
		...
		DupeMenuBase.Paint = function(_, w, h)
		...
		CloseDupeMenu.DoClick = function()
			DupeMenuBase:Remove()
		end
		...
		DupeList.OnRowSelected = function(_, _, line)
			Allow = true
			Data = line.Data
	...
	local function AAS_ReceiveFile() -- A little hijacking here and there from AdvDupe2 and we've now got an ez-dupe system for people to get public dupes from the server; discarded len and ply variables, incase needed later
	...
	do	-- Net
		...
		net.Receive("AAS.ReceiveCostScript", function()
			...
			for k in pairs(CostInfo.CalcSingleFilter) do
				local Str = "\"" .. k .. "\""
