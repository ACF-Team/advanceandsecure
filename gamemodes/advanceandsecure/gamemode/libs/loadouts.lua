...
if SERVER then
	...
	function NewPlyManager.BuildLoadout(OldLoadout, Loadout)
		...
		if Loadout.Gadget1 ~= "" then
			...
			if Gdgt.Type == "Kit" then
				for _, v in pairs(NewPlyManager.KitList[Loadout.Gadget1]) do
		...
		if Loadout.Gadget2 ~= "" then
			...
			if Gdgt.Type == "Kit" then
				for _, v in pairs(NewPlyManager.KitList[Loadout.Gadget2]) do
					SWEPList[#SWEPList + 1] = v
				end
			else
	...
	function PLAYER:ApplyLoadout()
		...
		for _, v in pairs(LoadoutData.give) do
			self:Give(v)
		end
	...
	local function LoadoutMenu()
		...
		LoadoutBase.Paint = function(_, w, h)
		...
		ArmorSlider.OnValueChanged = function(_, val)
		...
		LoadoutList.OnRowSelected = function(_, _, line)
			if SelectedTab == "Primary" then
