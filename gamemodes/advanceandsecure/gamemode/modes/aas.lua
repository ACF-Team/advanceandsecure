...
GMT.Save	= function(MapData) -- Return false to abort saving for any reason
	...
	if AAS.Funcs.GetSetting("Non-linear", false) == false then
		if not AAS.State.Data["Line"] then AAS.Funcs.Msg({Colors.ErrorCol, "[AAS] No point line defined, aborting."}) return false end
...
GMT.TicketThink	= function() -- Called when the server is doing ticket changes
	...
	if TotalPoints == 0 then AAS.Funcs.Stop() AAS.Funcs.Msg({Colors.ErrorCol, "[AAS] Halting game due to no capturable points being available."}) end
